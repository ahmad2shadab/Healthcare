  ...
  return (
    <div className="w-full lg:w-64 bg-white border-r border-gray-100 flex flex-col h-full">
      ...
      <div className="p-4 sm:p-6 flex-1">
        <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4 sm:mb-6">General</h2>
        <nav className="space-y-1 sm:space-y-2">
          {generalItems.map((item) => (
            <div
              key={item.label}
              className={`flex items-center gap-3 px-3 py-2 sm:py-2.5 rounded-lg text-sm font-medium transition-colors ${
                item.isActive ? "text-white rounded-lg" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"
              }`}
              style={item.isActive ? { backgroundColor: "#272666" } : {}}
            >
        ...
        <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4 sm:mb-6 mt-8">Tools</h2>
        <nav className="space-y-1 sm:space-y-2">
          {toolsItems.map((item) => (
            <div
              key={item.label}
